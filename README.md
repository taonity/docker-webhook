...
### Configuration 
 ...
 - `DEPLOY_HEALTHCHECK_TIMEOUT` - (optional) a non-negative integer number of seconds to wait for Docker health checks after `docker compose up -d`. When set to a value greater than `0`, the deploy script fails if any service exits early, becomes unhealthy, or does not become healthy before the timeout. When omitted or set to `0`, health-check waiting is skipped.
